...
namespace ai {
    ...
    void HeuristiqueAI::run (state::State& state, engine::Engine& engine)
    {
        ...
        std::vector<state::Entite*> listeEntites = state.getEntites();
        state::Entite* plusProcheEntite = listeEntites[1];
        int positionXEntite = listeEntites[0]->getPositionX();
        int positionYEntite = listeEntites[0]->getPositionY();
        int plusPetiteDistance = abs(listeEntites[1]->getPositionX() - positionXEntite) + abs(listeEntites[1]->getPositionY() - positionYEntite);
