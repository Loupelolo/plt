...
bool Scene::afficherFenetrePara(int conditionArret){
    sf::Clock clock;
    sf::Clock clock2;
    bool herosReste = false;
    ...
    while (m_window.isOpen())
    {
        if(m_caseActuelle.x == 0 && m_caseActuelle.y==0 && m_state->getOrdreTour()[0]->getType()<6){
            m_caseActuelle.x = m_state->getOrdreTour()[0]->getPositionX();
            m_caseActuelle.y = m_state->getOrdreTour()[0]->getPositionY();
        }
                                            ...
                                            for(unsigned int i = 0;i<entites.size() ;i++){
                                                ...
                                                if (m_caseActuelle == posEntite){
                                                    if(m_state->effectuerAction(m_actionSelectionnee)) {
                                                        ...
                                                        commandeJsonValue[m_state->getIndiceCommande()]["TypeId"] = "Action Supp";
                                                        commandeJsonValue[m_state->getIndiceCommande()]["Cible"] = entites[i]->getNom();
        ...
        if(conditionArret == 0 && m_state->getOrdreTour()[0]->getType()>=6 && herosReste){
            m_caseActuelle.x = 0;
            m_caseActuelle.y = 0;
            return true;
        } 
